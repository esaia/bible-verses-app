...
const Modal = ({ open, onClose, title, children, footer, width = 'max-w-3xl' }) => {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center p-3 font-ui sm:p-4"
    ...
    >
      ...
      >
        ...
        <div className="studio-scroll flex-1 overflow-y-auto px-4 py-2 sm:px-5">{children}</div>

        {footer && <div className="flex flex-wrap items-center justify-end gap-2 px-4 py-4 sm:px-5">{footer}</div>}
      </div>
    </div>
  );
};
